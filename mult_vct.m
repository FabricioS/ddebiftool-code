function [eig_mesh,eig_vec,eig_val]=mult_vct(rho,period,profile,mesh,degree,col,par)

% function [eig_mesh,eig_vec,eig_val]=mult_vct(rho,period,profile,mesh,degree,col,par)
% INPUT:
%	rho minimal modulus of eigenvalues of interest
...
% OUTPUT:
%       eig_mesh mesh for eigenvectors
%       eig_vec eigenvectors corresponding to multipliers with modulus > rho
%       eig_val eigenvalues in order of eig_vec

% (c) DDE-BIFTOOL v. 2.01, 21/09/2001
% Test version to find eigenfunctions for Kirk & Bernd!
...
for ll_i=1:ll
  ...
  if tt((ll_i-1)*mm+1)>=0
    ...
    for m_i=1:mm
      ...
      for k=1:mm+1,
        J(i_range,(index_a-1)*n+(k-1)*n+1:(index_a-1)*n+(k-1)*n+n)=J(i_range,(index_a-1)*n+(k-1)*n+1:(index_a-1)*n+(k-1)*n+n)+eye(n)*dPa(k);
      end;
      ...
      for t_i=1:d
        ...
        dPb(t_i,:)=poly_del(m,c_tau_trans)/hhh_tau(t_i); % NDDE addition

        % compute x_tau:

        x_tau(:,t_i)=psol_eva(profile,t,c_tau(t_i),m);
        dx_tau(:,t_i)=psol_dev(profile,t,c_tau(t_i),m);  % NDDE addition

      end;

      % determine A0:

      A0=sys_deri([x x_tau dx_tau/T],par,0,[],[]);

      % add -T*A0*Pa for da in J:

      for k=1:m+1
        J(i_range,(index_a-1)*n+(k-1)*n+1:(index_a-1)*n+(k-1)*n+n)=J(i_range,(index_a-1)*n+(k-1)*n+1:(index_a-1)*n+(k-1)*n+n)-T*A0*Pa(k);
      end;
      ...
      for t_i=1:d
        ...
        A1=sys_deri([x x_tau dx_tau/T],par,t_i,[],[]);

        % NDDE: compute A2
        if ndde
          A2 = sys_deri([x x_tau dx_tau/T],par,t_i + d,[],[]);
        else
          A2 = 0;
        end;
        ...
        for k=1:m+1
          % NDDE addition
          J(i_range,(index_b(t_i)-1)*n+(k-1)*n+1:(index_b(t_i)-1)*n+(k-1)*n+n)=J(i_range,(index_b(t_i)-1)*n+(k-1)*n+1:(index_b(t_i)-1)*n+(k-1)*n+n)-T*A1*Pb(t_i,k)-A2*dPb(t_i,k);
        end;
...
for k=1:length(s);
  ...
  if abs(s(k))>=rho,
    ...
    eig_val(next_row)=s(k);

    V=S1(:,k);

    vv=J(1:l1,l3+1:l2)\(-J(1:l1,1:l3)*V);

    eig_vec(next_row,1:n,1)=vv(l1-n+(1:n))/s(k);

    for j=1:l0
      eig_vec(next_row,1:n,j+1)=vv((j-1)*n+(1:n));
    end;
