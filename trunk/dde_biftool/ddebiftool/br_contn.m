function [branch,succ,fail,rjct]=br_contn(branch,max_tries)
...
if l<=1,
  err=l,
  error('BR_CONTN: could not start branch.');
end;
...
while kontinue & (tries<=max_tries | bound | bound_tau),
  ...
  if l==1,
    err=[tries-fail fail rjct],
    error('BR_CONTN: could not continue branch.');
  end;
  ...
  if tp_del~=0
    ...
    [delay_nr,t_z]=p_tsgn(last_point);
    if delay_nr~=0
      bound_tau=1;
    end;
  end;
  ...
  if l==2 | branch.method.continuation.prediction==1,
    ...
    if bound_tau %negative delay
      ...
      tau_n=p_tau(last_point,delay_nr,t_z);
      tau_p=p_tau(prev_point,delay_nr,t_z);
      del_tau=tau_p-tau_n;
    ...
    error('BR_CONTN: only linear prediction is currently implemented.');
  end;
  ...
  if bound,
    ...
    [new_point,success]=...
       p_correc(new_point,free_par,bound_secant,method,tries+1,prev_point);
  elseif bound_tau
      s=strcat('BR_CONTN warning: delay number_',num2str(delay_nr),' becomes negative.');
      disp(s);
    [new_point,success]=p_correc(new_point,free_par,[],method,0,[],delay_nr,t_z);
  else
    ...
    [new_point,success]=...
       p_correc(new_point,free_par,secant,method,tries+1,prev_point);
