...
\author{Maikel Bosschaert, Bram Wage, \\
Sebastiaan Janssens, Yuri Kuznetsov}
...
a bifurcation analysis toolbox running in \texttt{Matlab}\footnote{\href{http://www.mathworks.com}{http://www.mathworks.com}}
or \texttt{GNU Octave}\footnote{\href{http://www.gnu.org/software/octave}{http://www.gnu.org/software/octave}}.
For systems of DDEs with constant delays this extension is able to
automatically detect and accurately locate fold and Hopf bifurcations
...
to mimic those being used in MatCont\footnote{\href{https://sourceforge.net/projects/matcont/}{https://sourceforge.net/projects/matcont/}}.
...
\protect\caption{An overview of bifurcations and monitored test functions.\label{tab:List-of-bifurcations}}
\end{table}
...
\protect\caption{Bifurcation type numbering \label{tab:Bifurcation-type-numbering}scheme.}
\end{table}
...
\hline 
\end{tabular}\protect\caption{A list of normal form coefficients as stored in the \protect\blist{point.nmfm}
...
\]
where~$c_{1},c_{2},z\in\mathbb{C}$ with $\mbox{Re}\,c_{1}=0$. The
second Lyapunov coefficient is defined to be 
\begin{equation}
...
or \textquoteleft difficult\textquoteright{} dynamics for nearby parameter
values. Each case includes many subcases depending on the signs of
\[
...
\begin{figure}[h]
\begin{centering}
\includegraphics[scale=0.6]{images/nmfm_demo_01}\protect\caption{Plot of \protect\blist{branch1}\label{fig:Plot-of-branch1}}
...
\begin{figure}[h]
\centering{}\includegraphics[width=1\textwidth]{images/example1_stst_br_testfunctions}\protect\caption{Plot of test functions for \protect\blist{branch1} \label{fig:example_stst_br_testfunction}}
\end{figure}
...
\begin{lstlisting}[language=Matlab,basicstyle={\footnotesize},breaklines=true,tabsize=4]
...
\begin{figure}[h]
\centering{}\includegraphics[width=1\textwidth]{images/example1_hopf_branch1_testfunctions}\protect\caption{Plot of the test functions for \protect\blist{hopf\_branch1} \label{fig:test_functions_hopf_branch1}}
\end{figure}
...
\begin{figure}[H]
\centering{}\includegraphics[scale=0.4]{images/nmfm_demo_02}\protect\caption{Plot of detected bifurcations on the fold and Hopf branches. The asterisks
...
the absence of predation; $\alpha$ and $A$ stand for the predator
capturing rate and half saturation constant, respectively; $s$ is
the intrinsic growth rate of predator; however, carrying capacity
$x/b$ ($b$ is the conversion rate of prey into predators) is the
function on the population size of prey. The parameters $\alpha,A,\bar{m},\bar{h},s,b,$
and $\bar{\tau}$ are all positive constants. $\bar{m}$ is a constant
number of prey using refuges; $h$ is the rate of prey harvesting.
System (\ref{eq:Holling-Tanner-1}) can be transformed into
\begin{equation}
\left\{ \begin{array}{rl}
...
let $x=x-x_{\ast}$, $y=y-y_{\ast}$; then $P_{\ast}$ is translated
to $(0,0)$, and the linearization of system (\ref{eq:Holling-Tanner-1-1})
becomes (still denoting $\bar{x},\ \bar{y}$ as $x,\ y$)
\begin{equation}
...
\begin{figure}[h]
\centering{}\includegraphics[scale=0.4]{images/continuation_stst_branch_plot}\includegraphics[scale=0.4]{images/exm2_stst_br_testfunctions}\protect\caption{Left is a plot of the continuation of the steady-state point. In the
...
\centering{}\protect\caption{Left is a plot of the continuation of the steady-state point. At the
...
\protect\caption{Plot of the test function along\protect\blist{hopf\_branch}. \label{fig:example2_test_functions_hopf_branch}}
...
\protect\caption{Plot of the cycle emanating from the Hopf curve. At $\delta\approx0.577$
...
\protect\caption{Continuation of the homoclinic orbit in $(\beta,\delta)$.\label{fig:homoclinic-orbit}}
\end{figure}
...
\protect\caption{Multipliers for point number 10 from \protect\blist{psol\_branch}.\label{fig:exam2_multipliers}}
...
\protect\caption{Plot of Hopf, homoclinic and fold branch emanating from the Bogdanov-Takens
...
\begin{figure}[h]
\centering{}\includegraphics[width=1\textwidth]{images/exm3_stst_testfunctions}\protect\caption{Plot of test function for stst\_br\label{fig:stst_br_testfunctions}}
\end{figure}
...
\begin{figure}[h]
\centering{}\includegraphics[width=1\textwidth]{images/exm3_stst_testfunctions_smooth}\protect\caption{Plot of test functions for stst\_br\label{fig:stst_br_testfunctions_smooth}}
\end{figure}
...
\begin{figure}[h]
\centering{}\includegraphics[scale=0.6]{images/exm3_fold_branch}\protect\caption{Plot the fold branch. Here we see a cusp bifurcation at $(Q,E)=(1.6,0)$.\label{fig:fold_branch}}
\end{figure}
...
\centering{}\includegraphics[width=1\textwidth]{images/exm3_fold_testfunctions_small}\protect\caption{Plot of test functions for fold\_br\label{fig:fold_br_testfunctions}}
\end{figure}
